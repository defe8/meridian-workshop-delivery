<template>
  <div class="dashboard">
    ...
    <div v-else>
      ...
      <div class="kpi-section">
        ...
        <div class="kpi-grid">
          <div class="kpi-card">
            ...
            <div class="kpi-goal">{{ t('dashboard.kpi.goal') }}: 4.5 <span :class="['kpi-delta', deltaClass(-6.67)]">({{ formatDelta(-6.67) }})</span></div>
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-goal">{{ t('dashboard.kpi.goal') }}: {{ ordersData.goal }} <span :class="['kpi-delta', deltaClass(ordersFulfilledDelta)]">({{ formatDelta(ordersFulfilledDelta) }})</span></div>
            <div class="kpi-progress-bar">
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-goal">{{ t('dashboard.kpi.goal') }}: 95% <span :class="['kpi-delta', deltaClass(fillRate - 95)]">({{ formatDelta(fillRate - 95) }})</span></div>
            <div class="kpi-progress-bar">
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-goal">{{ t('dashboard.kpi.goal') }}: {{ formatCurrency(revenueGoal, selectedCurrency) }} <span :class="['kpi-delta', deltaClass(revenueDelta)]">({{ formatDelta(revenueDelta) }})</span></div>
            <div class="kpi-progress-bar">
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-goal">{{ t('dashboard.kpi.goal') }}: 3.0 <span :class="['kpi-delta', deltaClass(-6.67)]">({{ formatDelta(-6.67) }})</span></div>
            <div class="kpi-progress-bar">
      ...
      <!-- Pareto / ABC Analysis -->
      <div v-if="paretoData.items.length > 0" class="card pareto-card">
        <div class="card-header">
          <div>
            <h3 class="card-title">{{ t('dashboard.pareto.title') }}</h3>
            <span class="pareto-subtitle">{{ t('dashboard.pareto.subtitle') }}</span>
          </div>
          <div class="pareto-controls">
            <label class="pareto-control-label">{{ t('dashboard.pareto.metric') }}</label>
            <select v-model="paretoMetric" class="pareto-metric-select">
              <option value="orderValue">{{ t('dashboard.pareto.metricOrderValue') }}</option>
              <option value="itemCount">{{ t('dashboard.pareto.metricItemCount') }}</option>
            </select>
          </div>
        </div>
        <div class="pareto-summary-grid">
          <button
            type="button"
            class="pareto-class-card class-a"
            :class="{ active: paretoClassFilter === 'A', dimmed: paretoClassFilter !== 'all' && paretoClassFilter !== 'A' }"
            @click="setParetoClassFilter('A')"
            :aria-pressed="paretoClassFilter === 'A'"
          >
            <div class="pareto-class-label">{{ t('dashboard.pareto.classA') }}</div>
            <div class="pareto-class-stats">
              <div class="stat-row"><span class="stat-key">{{ t('dashboard.pareto.orders') }}</span><span class="stat-val">{{ paretoData.summary.A.count }} ({{ paretoData.summary.A.countPct.toFixed(1) }}%)</span></div>
              <div class="stat-row"><span class="stat-key">{{ t('dashboard.pareto.revenue') }}</span><span class="stat-val">{{ formatCurrency(Math.round(paretoData.summary.A.value), selectedCurrency) }} ({{ paretoData.summary.A.valuePct.toFixed(1) }}%)</span></div>
            </div>
          </button>
          <button
            type="button"
            class="pareto-class-card class-b"
            :class="{ active: paretoClassFilter === 'B', dimmed: paretoClassFilter !== 'all' && paretoClassFilter !== 'B' }"
            @click="setParetoClassFilter('B')"
            :aria-pressed="paretoClassFilter === 'B'"
          >
            <div class="pareto-class-label">{{ t('dashboard.pareto.classB') }}</div>
            <div class="pareto-class-stats">
              <div class="stat-row"><span class="stat-key">{{ t('dashboard.pareto.orders') }}</span><span class="stat-val">{{ paretoData.summary.B.count }} ({{ paretoData.summary.B.countPct.toFixed(1) }}%)</span></div>
              <div class="stat-row"><span class="stat-key">{{ t('dashboard.pareto.revenue') }}</span><span class="stat-val">{{ formatCurrency(Math.round(paretoData.summary.B.value), selectedCurrency) }} ({{ paretoData.summary.B.valuePct.toFixed(1) }}%)</span></div>
            </div>
          </button>
          <button
            type="button"
            class="pareto-class-card class-c"
            :class="{ active: paretoClassFilter === 'C', dimmed: paretoClassFilter !== 'all' && paretoClassFilter !== 'C' }"
            @click="setParetoClassFilter('C')"
            :aria-pressed="paretoClassFilter === 'C'"
          >
            <div class="pareto-class-label">{{ t('dashboard.pareto.classC') }}</div>
            <div class="pareto-class-stats">
              <div class="stat-row"><span class="stat-key">{{ t('dashboard.pareto.orders') }}</span><span class="stat-val">{{ paretoData.summary.C.count }} ({{ paretoData.summary.C.countPct.toFixed(1) }}%)</span></div>
              <div class="stat-row"><span class="stat-key">{{ t('dashboard.pareto.revenue') }}</span><span class="stat-val">{{ formatCurrency(Math.round(paretoData.summary.C.value), selectedCurrency) }} ({{ paretoData.summary.C.valuePct.toFixed(1) }}%)</span></div>
            </div>
          </button>
        </div>
        <div v-if="paretoClassFilter !== 'all'" class="pareto-filter-banner">
          <span>{{ t('dashboard.pareto.filterActive', { cls: paretoClassFilter }) }}</span>
          <button type="button" class="pareto-filter-clear" @click="setParetoClassFilter(paretoClassFilter)">
            {{ t('dashboard.pareto.clearFilter') }}
          </button>
        </div>
        <div class="pareto-chart-wrapper">
          <div class="pareto-chart-grid">
            <div class="pareto-y-label-left">
              {{ paretoMetric === 'itemCount' ? t('dashboard.pareto.metricItemCount') : t('dashboard.pareto.metricOrderValue') }}
            </div>
            <div class="pareto-chart-area">
              <svg viewBox="0 0 100 100" preserveAspectRatio="none" class="pareto-chart">
                <!-- 80% and 95% threshold lines -->
                <line x1="0" y1="20" x2="100" y2="20" stroke="#94a3b8" stroke-width="0.2" stroke-dasharray="0.8 0.6"/>
                <line x1="0" y1="5" x2="100" y2="5" stroke="#cbd5e1" stroke-width="0.2" stroke-dasharray="0.8 0.6"/>
                <!-- Bars (top 60 orders) -->
                <g>
                  <rect
                    v-for="(b, i) in paretoBars"
                    :key="b.id"
                    :x="(i / Math.max(1, paretoBars.length)) * 100"
                    :y="100 - (b.value / paretoMaxValue) * 80"
                    :width="(100 / Math.max(1, paretoBars.length)) * 0.85"
                    :height="(b.value / paretoMaxValue) * 80"
                    :class="['pareto-bar', `bar-${b.cls.toLowerCase()}`, { 'bar-dimmed': paretoClassFilter !== 'all' && paretoClassFilter !== b.cls }]"
                  />
                </g>
                <!-- Cumulative curve -->
                <path :d="paretoCurvePath" fill="none" stroke="#0f172a" stroke-width="0.5" vector-effect="non-scaling-stroke"/>
              </svg>
              <div class="pareto-axis-labels">
                <span class="threshold-label threshold-80">80%</span>
                <span class="threshold-label threshold-95">95%</span>
              </div>
            </div>
            <div class="pareto-y-label-right">{{ t('dashboard.pareto.cumulative') }}</div>
            <div class="pareto-x-label">{{ t('dashboard.pareto.xAxis') }}</div>
          </div>
          <div class="pareto-legend">
            <span class="legend-item"><span class="legend-dot bar-a"></span>{{ t('dashboard.pareto.classA') }}</span>
            <span class="legend-item"><span class="legend-dot bar-b"></span>{{ t('dashboard.pareto.classB') }}</span>
            <span class="legend-item"><span class="legend-dot bar-c"></span>{{ t('dashboard.pareto.classC') }}</span>
            <span class="legend-item"><span class="legend-line"></span>{{ t('dashboard.pareto.cumulative') }}</span>
          </div>
        </div>
      </div>
...
export default {
  ...
  setup() {
    ...
    // KPI delta helpers — green for positive, red for negative
    const ordersFulfilledDelta = computed(() => {
      if (!ordersData.value.goal) return 0
      return ((ordersData.value.fulfilled / ordersData.value.goal - 1) * 100)
    })

    const revenueDelta = computed(() => {
      if (!revenueGoal.value) return 0
      return ((summary.value.total_orders_value / revenueGoal.value - 1) * 100)
    })
    ...
    // Pareto / ABC analysis on orders
    // Tangent-method approximation: cumulative thresholds at 80% (A) and 95% (B)
    // Available metrics on the Y axis (and the variable used for ranking)
    const paretoMetric = ref('orderValue')
    const paretoClassFilter = ref('all') // 'all' | 'A' | 'B' | 'C'

    const setParetoClassFilter = (cls) => {
      paretoClassFilter.value = paretoClassFilter.value === cls ? 'all' : cls
    }

    const paretoMetricExtractor = (order) => {
      if (paretoMetric.value === 'itemCount') {
        const items = order.items || []
        return items.reduce((s, it) => s + (it.quantity || 0), 0)
      }
      // default: order value (revenue)
      return order.total_value || 0
    }

    const paretoData = computed(() => {
      const orders = allOrders.value
        .filter(o => o && paretoMetricExtractor(o) > 0)
        .map(o => ({ id: o.order_number || o.id, value: paretoMetricExtractor(o) }))
        .sort((a, b) => b.value - a.value)

      const total = orders.reduce((s, o) => s + o.value, 0)
      if (!total) return { items: [], summary: null, total: 0 }
      ...
      const items = orders.map((o, i) => {
        ...
        return { ...o, rank: i + 1, cumPct, cls }
      })

      const summary = { A: { count: 0, value: 0 }, B: { count: 0, value: 0 }, C: { count: 0, value: 0 } }
      items.forEach(it => {
        summary[it.cls].count += 1
        summary[it.cls].value += it.value
      })
      const totalCount = items.length
      Object.keys(summary).forEach(k => {
        summary[k].countPct = totalCount ? (summary[k].count / totalCount) * 100 : 0
        summary[k].valuePct = total ? (summary[k].value / total) * 100 : 0
      })

      return { items, summary, total }
    })

    const paretoMaxValue = computed(() => {
      const items = paretoData.value.items
      return items.length ? items[0].value : 1
    })

    const paretoBars = computed(() => {
      // Cap visible bars to keep the chart readable
      const items = paretoData.value.items
      const cap = 60
      return items.slice(0, cap)
    })

    const paretoCurvePath = computed(() => {
      const items = paretoData.value.items
...
.pareto-class-card:hover {
  background: var(--bg-hover);
  border-color: var(--border-strong);
  transform: translateY(-1px);
}

.pareto-class-card:focus {
  outline: none;
}

.pareto-class-card:focus-visible {
  box-shadow: 0 0 0 3px var(--accent-soft);
}

.pareto-class-card.active {
  background: var(--bg-surface);
  box-shadow: var(--shadow-md);
  border-color: var(--border-strong);
}

.pareto-class-card.class-a.active { border-left-color: #059669; box-shadow: 0 4px 14px rgba(5, 150, 105, 0.18); }
.pareto-class-card.class-b.active { border-left-color: #f59e0b; box-shadow: 0 4px 14px rgba(245, 158, 11, 0.2); }
.pareto-class-card.class-c.active { border-left-color: #475569; box-shadow: 0 4px 14px rgba(71, 85, 105, 0.18); }

.pareto-class-card.dimmed {
  opacity: 0.45;
}

.pareto-class-card.class-a { border-left-color: #059669; }
.pareto-class-card.class-b { border-left-color: #f59e0b; }
.pareto-class-card.class-c { border-left-color: #94a3b8; }

.pareto-filter-banner {
  display: flex;
  align-items: center;
  justify-content: space-between;
...
.pareto-filter-clear:hover {
  background: var(--accent);
  color: white;
}

.pareto-bar.bar-dimmed {
  opacity: 0.12;
}
...
.pareto-class-stats .stat-key { color: var(--fg-muted); }
.pareto-class-stats .stat-val { color: var(--fg-primary); font-weight: 600; }
...
.pareto-bar.bar-a { fill: #059669; opacity: 0.85; }
.pareto-bar.bar-b { fill: #f59e0b; opacity: 0.85; }
.pareto-bar.bar-c { fill: #94a3b8; opacity: 0.85; }
...
.threshold-label.threshold-80 { top: calc(20% - 0.5rem); }
.threshold-label.threshold-95 { top: calc(5% - 0.5rem); }
...
.pareto-legend .legend-item {
  display: inline-flex;
  align-items: center;
  gap: 0.375rem;
}

.pareto-legend .legend-dot {
  width: 10px;
  height: 10px;
  border-radius: 2px;
}

.pareto-legend .legend-dot.bar-a { background: #059669; }
.pareto-legend .legend-dot.bar-b { background: #f59e0b; }
.pareto-legend .legend-dot.bar-c { background: #94a3b8; }

.pareto-legend .legend-line {
  width: 16px;
  height: 2px;
  background: #0f172a;
}
